...
func (tData *TransferData) moveOffTelscope() {
	...
	// What files have not been copied to locations where active rules exist
...
func connectToObjectStorage(location string) (ObjectStorageConnection, error) {
	...
	if queryReturn, err := dbCon.QueryRead(query, &datasets.LocationsTable{}); err != nil {
		return ObjectStorageConnection{}, err
		...
		return ObjectStorageConnection{}, fmt.Errorf(errMsg)
	}

	// Use the data from the SQL query to populate the ObjectMetadata struct
	transferData := ObjectStorageConnection{
...
func (tData *TransferData) copyFileToExternal(src datasets.FilesThatNeedToBeBackedUp) error {
	...
	// See if bucket exists (must be lowercase with no whitespace), else create
	bucket := strings.ToLower(strings.Replace(src.BucketName, " ", "", -1))
	if ok, err := remoteObjSrg.minioClient.BucketExists(ctx, bucket); err != nil {
		return err
	} else if ok == false {
	...
	// err = localObjSrg.minioClient.FGetObject(ctx, "fyst", src.FileName, tempFile, minio.GetObjectOptions{})
	// if err != nil {
	// 	return err
	// }

	// // Upload the zip file
	// _, err = remoteObjSrg.minioClient.FPutObject(ctx, bucket, src.FileName, tempFile, minio.PutObjectOptions{ContentType: "application/zip"})
	// if err != nil {
	// 	return err
	// }
	...
	// Stream object from local object storage to off-site
	object, err := localObjSrg.minioClient.GetObject(ctx, "fyst", src.FileName, minio.GetObjectOptions{})
	if err != nil {
		return err
	}

	_, err = remoteObjSrg.minioClient.PutObject(ctx, bucket, src.FileName, object, -1, minio.PutObjectOptions{ContentType: "application/octet-stream"})
	if err != nil {
		return err
	}
