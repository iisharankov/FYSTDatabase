...
func verifyObject(filename string) error {
	...
	// v // Connect to minio instance known as "Observatory" and gets stats of object "filename"
	temp, err := connectToObjectStorage("Observatory")
	objInfo, err := temp.minioClient.StatObject(temp.ctx, "fyst", filename, minio.StatObjectOptions{})
	if err != nil {
		return err
	}
	// ^ //

	// v // The next block queires the Files table for what the MD5 is for the given FileName to
	...
	// v // Check that MD5Sum given in initial file request matches the MD5Sum from the uploaded object
